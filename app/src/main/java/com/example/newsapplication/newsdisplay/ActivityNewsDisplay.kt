...
import com.example.newsapplication.service.NewsLoadService
...
class ActivityNewsDisplay : AppCompatActivity(), NewsResponseObserver, ItemClickObserver {
    ...
    private fun initializeViewModel() {
        viewModelNewsDisplay = ViewModelNewsDisplay(this@ActivityNewsDisplay)
        viewModelNewsDisplay.addObserver(this)
    }

    private fun bindToNewsDataLoadService() {
        Intent(this, NewsLoadService::class.java).also {
            bindService(it, newsServiceConnection, Context.BIND_AUTO_CREATE)
            Log.i("Service", "Service bind initiated")
        }
